...
router.get("/pasing/:cur", function (req, res) {
    ...
    getConnection().query(queryString, function(error2, data) {
        ...
        fs.readFile('list.html', 'utf-8', function(error, data) {
            ...
            var queryString = 'select * from channel order by name desc limit ?,?';
...
// 글 상세보기
router.get("/detail/:email", function(req, res) {
    console.log("수정 진행")
    
    fs.readFile('detail.html', 'utf-8', function(error, data) {
      getConnection().query('select * from channel where email = ?', [req.params.email], function(error, result) {
          res.send(ejs.render(data, {
              data : result[0]
          }))
      })
