    ...
    if(database) {
        authUser(database, paramId, paramPassword, function(err, docs) {
            ...
            if(docs) {
                ...
                // 조회 결과에서 사용자 이름 확인
                var nickname = docs[0].nickname;
                
                res.writeHead('200', {'Content-Type':'text/html;charset=utf8'});
                res.write('<h1>로그인 성공</h1>');
                res.write('<div><p>사용자 아이디 : ' + paramId +'</p></div');
                res.write('<div><p>사용자 닉네임 : ' + nickname + '</p></div>');
                res.write("<br><br><a href = '/css/mainpage.html'>메인 페이지가기</a>");
                res.end();
...
router.route('/process/signup').post(function(req, res) {
    ...
    console.log('요청 파라미터 : ' + paramId + ', ' + paramPassword + ',' + paramNickname + ', ' + paramMail);
    
    // 데이터베이스 객체가 초기화된 경우, addUser 함수 호출하여 사용자 추가
    if(database) {
        addUser(database, paramId, paramPassword, paramNickname, paramMail, function(err, result) {
            if(err) {throw err;}
...
var authUser = function(database, id, password, callback) {
    ...
    users.find({"id":id, "password":password}).toArray(function(err, docs) {
        if(err) { // 에터 발생 시 콜백 함수를 호출하면서 에러 객체 전달
            callback(err, null);
            return;
        }
        ...
        callback(null, docs);    
        } else { // 조회한 레코드가 없는 경우 코백 함수를 호출하면서 null, null 전달
...
// 사용자를 추가하는 함수
var addUser = function(database, id, password, nickname, mail, callback) {
    console.log('addUser 호출됨 : ' + id + ', ' + password + ', ' + nickname + ', ' + mail);
    
    // users 컬렉션 참조
    var users = database.collection('users');
    
    // id,password,nickname,mail을 이용해 사용자 추가
    users.insertMany([{"id":id, "password":password, "nickname":nickname, "mail":mail}], function(err, result) {
