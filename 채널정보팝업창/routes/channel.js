...
var saveChannel = function(req, res) {
    ...
    var paramIntroduction = req.body.introduction || req.query.introduction;
...
router.get("/info/:name", function(req, res) {
        console.log("크리에이터 정보 조회")
        
    var queryString = 'select * from channel where name = ?'
    getConnection().query(queryString, [req.params.name], function(error, result) {
        if(error) {
...
// 글 상세보기
router.get("/info/:name", function(req, res) {
    console.log("채널 상세정보 열람")
    
    fs.readFile('creatorInfor.html', 'utf-8', function(error, data) {
